...
static void	zoom_x_y_plus(int key, t_fdf *p)
{
	if (p->shift > KZ && p->flag == 0)
	{
		if (key == 4 && p->mouse_x < WIDHT / 2)
		{
			...
			{
				p->x0 -= (KZ_MAX * (p->width - 1) / 2 * cos(ANGLE));
				p->y0 += (KZ_MAX * (p->width - 1) / 2* sin(ANGLE));
			}
			if (p->mouse_y > HIGHT /2)
			{
				p->x0 -= (KZ_MAX * (p->hight - 1) / 2 * cos(ANGLE));
				p->y0 -= (KZ_MAX * (p->hight - 1) / 2 * sin(ANGLE));
			}
		}
		if (key == 4 && p->mouse_x > WIDHT / 2)
		{
			if (p->mouse_y < HIGHT /2)
			{
				p->x0 -= (KZ_MAX * (p->width - 1 + (p->hight - 1) / 2) * cos(ANGLE));
				p->y0 += (KZ_MAX * (p->width - 1 - (p->hight - 1) / 2) * sin(ANGLE));
			}
			if (p->mouse_y > HIGHT /2)
			{
				p->x0 -= (KZ_MAX * (p->hight - 1 + (p->width - 1) / 2) * cos(ANGLE));
				p->y0 -= (KZ_MAX * (p->hight - 1 - (p->width - 1) / 2) * sin(ANGLE));
			}
		}
	}
